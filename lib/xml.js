  ...
  var toXml = function(obj, depth) {
    return _.join(_.map(obj, function(value, key) {
      ...
      if (_.isArray(value)) {
        ...
        // If key has a prefix it will override defaults below
        ...
        if (_.includes(zObjectMethods, method) && _.includes(_.map(objectList, function(obj) {return obj.name.toLowerCase();}), name.toLowerCase())) {
          if (depth === 1) {
            name = 'zObjects';
            prefix = ns1;
        ...
        if (_.isObject(value)) {
          // if we're in a zObject then at depth 1 we want ns2
          // else we probably want zns/ns1 at depth 1
          // TODO: check WSDL ?
//          prefix = prefix || (depth === 1 ? ns2 : ns1);
          //prefix = prefix || (depth > 2 ? ns1 : ns2); // pretty good: DM mar 6, 2016
          prefix = prefix || ns1;
          console.log(depth, name, prefix)
